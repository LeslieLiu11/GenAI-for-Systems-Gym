...
# usage: eval_task4_bs.sh <benchmark> <tracelen> <checkpoint> <policy> <batchsize> [opt:testcsv]
# 例如：
#   ./eval_task4_bs.sh astar 313B 20 0 32
# 這表示：
#   - benchmark=astar
#   - tracelen=313B
#   - checkpoint=20（即讀取 20000.ckpt）
#   - policy=0（learned）
#   - batchsize=32（對應資料夾 mlp_bs_32）
#   - 若不提供 testcsv，則預設為 "<benchmark>_<tracelen>_test"

if [ $# -lt 5 ]; then
    echo "Usage: $0 <benchmark> <tracelen> <checkpoint> <policy> <batchsize> [opt:testcsv]"
    exit 1
fi
...
# 假設訓練時資料夾名稱為 mlp_bs_<batchsize>
traindir="astar_batch_${batchsize}"
...
echo "#BSUB -J hw4_${bm}_${pol}_bs${batchsize}"
echo ""
echo "source ~/.bashrc"
...
echo "  --experiment_name=\"${bm}${len}_chkpt_bs${batchsize}\" \\"
